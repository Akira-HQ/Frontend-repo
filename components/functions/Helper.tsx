...
const InputField: React.FC<{ label: string; type: string; value: string; onChange: (e: React.ChangeEvent<HTMLInputElement>) => void; icon: LucideIcon; placeholder?: string }> = ({ label, type, value, onChange, icon: Icon, placeholder }) => (
...
const PasswordRequirement: React.FC<{ text: string; passed: boolean }> = ({ text, passed }) => (
  <li className={`flex items-center text-sm ${passed ? 'text-red-400' : 'text-green-400'}`}>
    {passed ? <XCircle className="w-4 h-4 mr-2" /> : <CheckCircle className="w-4 h-4 mr-2" />}
    {text}
  </li>
);

const PlatformButton: React.FC<{ icon: LucideIcon; name: string; onClick: () => void; platform: string }> = ({ icon: Icon, name, onClick, platform }) => (
  <button
    type="button"
    onClick={onClick}
    className={`
    ...
    `}
  >
    <Icon className={`w-10 h-10 mb-2 ${platform === name.toLowerCase() ? 'text-[#00A7FF]' : 'text-gray-400'}`} />
