...
export const PricingToggle: React.FC<{
  ...
  <div className="flex justify-center items-center mb-10 select-none">
    ...
    <span
      className={`mr-4 font-semibold text-xl transition-colors duration-300 ${!monthly ? "text-white" : "text-gray-500"}`}
    >
      Yearly (Save 20%)
    </span>
    <button
      onClick={() => setMonthly(!monthly)}
      className={`relative inline-flex items-center h-8 w-16 rounded-full transition-all duration-300 shadow-inner ${
        monthly ? "bg-gray-800" : NEON_GRADIENT
      }`}
    >
      <span
        aria-hidden="true"
...
        `}
    ...
    {/* Removed drop-shadow-lg for a gentler appearance */}
    <span
      className={`ml-4 font-semibold text-xl transition-colors duration-300 ${monthly ? "text-white" : "text-gray-500"}`}
    >
      Monthly
    </span>
  </div>
...
export const PricingCard: React.FC<PricingCardProps> = ({
  ...
  const hoverShadow = isFeatured
    ? "hover:shadow-[0_0_40px_rgba(165,0,255,0.9)]"
    : "hover:shadow-[0_0_40px_rgba(255,179,0,0.5)]";
...
      `}
    >
      ...
      <div
        className={`absolute w-full top-0 z-10 h-[300px] left-0 bg-gradient-to-b ${glowColor} to-transparent opacity-30 group-hover:opacity-40 transition duration-500`}
      ></div>
