...
const ForgotPasswordContent: React.FC = () => {
  ...
  const handleResetPassword = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setAlertMessage(
        error.message ||
          "Failed to reset password. Check your code or try again.",
        "error",
      );
      addToast(
        error.message ||
  ...
  const Step2Form = (
    <form onSubmit={handleResetPassword} className="space-y-6">
      ...
      <p className="text-center text-gray-400 pb-4">
        ...
        <span className="text-[#00A7FF] font-medium">{email}</span> and set your
        new password.
      </p>
      ...
      <PrimaryButton
        ...
        className={`mt-5 w-full font-bold tracking-wide`}
        disabled={
          loading || !isPasswordStrong || newPassword !== confirmNewPassword
        }
      >
        {loading ? (
          <span className="flex items-center justify-center space-x-2">
