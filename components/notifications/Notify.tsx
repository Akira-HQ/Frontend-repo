...
interface NotifyProps {
  ...
  currentIndex: number;
  totalUnread: number;
  // Pass the raw quota data from the Quota DB fetch here
  liveQuotas?: any[];
}
...
export default function Notify({
  ...
  // We find the right data from the DB array based on the notification ID
  const quotaData = liveQuotas.find(q =>
    (notification.id.includes('audit') && q.id === 'deep_audits') ||
    (notification.id.includes('chat') && q.id === 'customer_chats')
  );
  ...
  const getQuotaStatus = () => {
    ...
    return { color: "text-blue-500", bg: "bg-blue-500", val: remaining };
  };

  const status = getQuotaStatus();

  const getNotificationIcon = (type: Notification["type"]) => {
    if (status.icon && notification.type === "USAGE_ALERT") return status.icon;
    switch (type) {
      case "USAGE_ALERT":
