...
const PageTracker = () => {
  useEffect(() => {
    ...
    const pageHistory: PageHistory[] = existingData
      ? JSON.parse(existingData)
      : [];

    const existingPageIndex = pageHistory.findIndex(
    ...
    const handleBeforeUnload = () => {
      const updatedPageHistory: PageHistory[] = JSON.parse(
        localStorage.getItem("pageHistory") || "[]",
      );
      const currentPageIndex = updatedPageHistory.findIndex(
        (page) => page.page === currentPage,
      );
      if (currentPageIndex !== -1) {
        const exitTime = new Date().toISOString();
        const entryTime = new Date(
          updatedPageHistory[currentPageIndex].entryTime,
        ).getTime();
