...
import { TrendingUp, Zap, Clock, AlertTriangle, MessageCircle, DollarSign, XCircle } from 'lucide-react';
...
const MOCK_QUOTAS: QuotaItem[] = [
  { id: 'messages', label: 'AI Messages (Monthly)', used: 4500, limit: 10000, unit: 'messages' },
  { id: 'rag_queries', label: 'RAG Queries (Monthly)', used: 800, limit: 1000, unit: 'queries' },
  { id: 'compute', label: 'Proactive Compute Time', used: 32, limit: 40, unit: 'hours' },
];

const MOCK_SERVICES: ServiceStatus[] = [
  { id: 'gemini_api', label: 'Gemini AI Core API', status: 'Operational', detail: 'Primary conversational engine latency: < 50ms.' },
  { id: 'webhook_sync', label: 'Shopify Webhook Sync', status: 'Operational', detail: 'Real-time sync active.' },
  { id: 'rate_limit', label: 'API Rate Limit Status', status: 'Operational', detail: 'Currently running below 50% threshold.' },
];
// --- END MOCK DATA ---
  ...
  return (
    <div className='p-4 bg-gray-900 rounded-xl border border-gray-800 shadow-md'>
      <h3 className='text-sm text-gray-400 font-medium flex items-center justify-between'>
        {item.label}
        <span className={`text-xs font-semibold ${percentage >= 90 ? 'text-red-400' : 'text-gray-300'}`}>
...
const ServiceStatusItem: React.FC<{ service: ServiceStatus }> = ({ service }) => {
  ...
  return (
    <div className='p-3 bg-gray-900 rounded-lg border border-gray-800 flex items-center space-x-3'>
      ...
      <span className='text-xs text-gray-500 max-w-[50%] truncate'>{service.detail}</span>
    </div>
  );
};
    ...
    <div className='space-y-8'>
      ...
      <div className='bg-[#0b0b0b] rounded-xl p-6 shadow-2xl border border-gray-800'>
        <h2 className='text-xl font-bold mb-6 border-b border-gray-800 pb-3 flex items-center gap-2'>
          <MessageCircle className='w-5 h-5 text-[#FFB300]' /> AI Usage & Quota Management (Growth Plan)
        </h2>
        ...
        <div className='mt-8 pt-4 border-t border-gray-800 flex justify-between items-center'>
          <p className='text-sm text-gray-400'>
            Approaching monthly limits? Upgrade now to prevent service degradation.
          </p>
          <a
            href='/pricing'
      ...
      <div className='bg-[#0b0b0b] rounded-xl p-6 shadow-2xl border border-gray-800'>
        <h2 className='text-xl font-bold mb-6 border-b border-gray-800 pb-3 flex items-center gap-2'>
          <TrendingUp className='w-5 h-5 text-green-400' /> Real-Time Service Health
        </h2>
