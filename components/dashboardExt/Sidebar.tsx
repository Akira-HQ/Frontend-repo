...
import { IoBarChart, IoChatbubbleOutline, IoGitNetworkOutline, IoSettingsSharp } from 'react-icons/io5'
...
const Sidebar = ({ isCollapsed, onToggle, width, onMouseDown }: SidebarProps) => {
  ...
  const handleDismiss = (id: string) => {
    setAllNotifications(currentNotifications => currentNotifications.map(n => n.id === id ? { ...n, read: true } : n))
    setCurrentIndex(0)
  }

  const handleNext = () => {
    setCurrentIndex(prevIndex => Math.min(prevIndex + 1, unReadNotifications.length - 1))
  }
