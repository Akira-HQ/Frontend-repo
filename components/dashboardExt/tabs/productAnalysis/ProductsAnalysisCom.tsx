...
const ProductsAnalysisCom = () => {
  ...
  return (
    <div className="mt-20 ml-6">
      {analysisSummary && (
        <div
          className={`analysis px-4 py-2 overflow-hidden transition-all duration-300 ease-in bg-[#0b0b0b] rounded-2xl w-[450px] flex flex-col ${
            analysisView === "closed"
              ? "max-h-[40px]"
        ...
        >
          <div className="flex justify-between items-center flex-shrink-0">
            ...
            {analysisView === "closed" ? (
              ...
              <BiChevronUp
                className="text-2xl cursor-pointer"
                onClick={() => setAnalysisView("closed")}
              />
            )}
          </div>
          <hr className="my-2 opacity-15 flex-shrink-0" />{" "}
          {/* Added flex-shrink-0 */}
          {/* --- SUMMARY VIEW (Unchanged, will fit in max-h-48) --- */}
          {analysisView === "summary" && (
            <div className="animate-fade-in">
              <p>
                Analyzed {analysisSummary.totalProducts} products from your
                store.
              </p>
              <p>
                {analysisSummary.strongCount}/{analysisSummary.totalProducts}{" "}
                meet training criteria.
              </p>

              <div className={`h-2 bg-white mt-2 rounded-3xl overflow-hidden`}>
                <div
                  className={`h-2 bg-green-600 rounded-3xl`}
                  style={{ width: `${analysisSummary.healthScore}%` }}
                ></div>
              </div>
              ...
              <div className="actions mt-3 flex justify-between">
                ...
                >
                  <HiSparkles className="text-yellow-300" /> View{" "}
                  {analysisSummary.weakCount} weak products
                </button>
                <button className="flex items-center gap-1 px-3 py-1 rounded-lg shadow-lg bg-[#0f1117] cursor-pointer">
                  <HiSparkles className="text-yellow-300" /> Test strong
                  products
                </button>
              </div>
            </div>
            ...
            <div className="animate-fade-in flex flex-col min-h-0">
              <div className="flex items-center gap-2 mb-3 flex-shrink-0">
                ...
                <h4 className="font-semibold text-white">
                  Weak Products ({analysisSummary.weakCount})
                </h4>
              </div>
              ...
              <div className="flex flex-col gap-3 overflow-y-auto pr-2">
                {analyzedProducts
                  ...
                  .map((product) => (
                    <div
                      key={product.id}
                      className="bg-[#0f1117] p-3 rounded-lg"
                    >
