"use client"
import { Package, Tag, Zap, Code, Barcode, DollarSign, Loader2 } from 'lucide-react';
...
const OverviewProductCardGrid = ({ products, onSelectedProduct }: OverviewProductCardGridProps) => {
  ...
  const getStockStatus = (stock: number) => {
    if (stock === 0) return { label: 'Out of Stock', color: 'text-red-400', bg: 'bg-red-900/30', icon: Code };
    if (stock < 10) return { label: 'Low Stock', color: 'text-yellow-400', bg: 'bg-yellow-900/30', icon: Tag };
    return { label: 'In Stock', color: 'text-green-400', bg: 'bg-green-900/30', icon: Package };
  };

  return (
    <div className='mt-8'>
      <h2 className='text-xl font-bold text-white mb-4'>Full Catalog Overview ({products.length} Items)</h2>
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        {products.map((product) => {
          const stockInfo = getStockStatus(product.stock);
          const healthColor = product.health > 65 ? 'bg-[#A500FF]' : 'bg-[#FFB300]';
            ...
            >
              ...
              <div className="h-40 bg-gray-900 flex items-center justify-center relative">
                {product.imageUrls && product.imageUrls.length > 0 ? (
                  <img
                    src={product.imageUrls[0] || 'https://placehold.co/400x160/1f2937/FFFFFF?text=No+Image'}
                    alt={product.name}
                    className='w-full h-full object-cover opacity-90 group-hover:opacity-100 transition duration-300'
                    onError={(e) => { e.currentTarget.onerror = null; e.currentTarget.src = 'https://placehold.co/400x160/1f2937/FFFFFF?text=No+Image'; }}
                  />
                ) : (
              ...
              <div className='p-4 flex-grow'>
                ...
                <div className='flex justify-between items-center text-sm mb-3'>
                  <span className='text-xs text-gray-500 font-mono flex items-center gap-1'>
                    <Barcode className='w-3 h-3' /> {product.sku || product.id.substring(0, 8)}
                  </span>
